...
WESTON_APPS_CONF_OPTS += -DSTAGING_DIR=$(STAGING_DIR)

ifeq ($(BR2_PACKAGE_KYLIN_HDMI_INFO),y)
WESTON_APPS_DEPENDENCIES += udev kylin-platform-lib kylin-graphics
WESTON_APPS_CONF_OPTS += -DENABLE_KYLIN_HDMI_INFO=ON
else
WESTON_APPS_CONF_OPTS += -DENABLE_KYLIN_HDMI_INFO=OFF
endif

ifeq ($(BR2_PACKAGE_KYLIN_SIMPLE_APP),y)
WESTON_APPS_DEPENDENCIES += udev kylin-platform-lib kylin-graphics
WESTON_APPS_CONF_OPTS += -DENABLE_KYLIN_SIMPLE_APP=ON
else
WESTON_APPS_CONF_OPTS += -DENABLE_KYLIN_SIMPLE_APP=OFF
endif

#WESTON_APPS_DEPENDENCIES = kylin-weston
